#!/usr/bin/env python3
# This is the script to send alerts from Icinga2 to TaskCall.
...
config_parameters = {
    ...
    'log_path': '/var/log/taskcall-icinga2/send_to_taskcall.log',
    'icinga2_server': ICINGA2_SERVER,
    'icinga2_to_taskcall.logger': 'warning',
    'taskcall.api.url': 'https://integrations.taskcallapp.com/icinga2/',
    'icinga2_to_taskcall.http.proxy.enabled': 'false',
    'icinga2_to_taskcall.http.proxy.port': '1111',
    'icinga2_to_taskcall.http.proxy.host': 'localhost',
    'icinga2_to_taskcall.http.proxy.protocol': 'http',
    'icinga2_to_taskcall.http.proxy.username': '',
    'icinga2_to_taskcall.http.proxy.password': ''
}
config_path = '/home/taskcall-icinga2/icinga2_to_taskcall.conf'
    ...
    try:
        ...
        for line in file_lines:
            ...
            if '#' not in line and line != '' and len(line) != 0:
                ...
                if key == 'icinga2_to_taskcall.timeout':
                    value = int(value)
...
def send_post_request():
    '''
    Send a HTTP post request to TaskCall's server to process alerts generated by Icinga2.
    '''
    succeeded = False
    attempts = 0
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='send events Icinga2 to TaskCall')

    parser.add_argument('-integKey', help='Integration key issued by TaskCall')
    parser.add_argument('-icinga2Server', default=config_parameters['icinga2_server'], help='Icinga2 server')
    ...
    parser.add_argument('-sc', help='SERVICECHECKCOMMAND', default=None)
    ...
    cmd_line_arguments['icinga2_server'] = args.icinga2Server
    ...
    if cmd_line_arguments['notification_type'] is None or cmd_line_arguments['notification_type'] == '':
        logging.error('Icinga2 NOTIFICATIONTYPE parameter is missing. Aborting process.')
    else:
        send_post_request()
